...
const CartItem = () => {
  ...
  const handleDelete = async (itemId) => {
    try {
      ...
      const response = await axios.delete(`${process.env.REACT_APP_BACKEND_URL}:5000/api/cart/${itemId}`);
  ...
  const handleDecrementChange = async (id, qty) => {
    if (qty <= 1) {
        ...
        try {
            ...
            await axios.patch(`${process.env.REACT_APP_BACKEND_URL}:5000/api/cart/${id}`, {
                quantity: qty - 1
            });
